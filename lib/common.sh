#!/bin/bash
# dockweb - common helpers

# Colors
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
CYAN='\033[0;36m'
BOLD='\033[1m'
DIM='\033[2m'
NC='\033[0m'

# Project root (where dockweb lives)
DOCKWEB_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

log_info()    { echo -e "${BLUE}[INFO]${NC} $*"; }
log_success() { echo -e "${GREEN}[OK]${NC} $*"; }
log_warn()    { echo -e "${YELLOW}[WARN]${NC} $*"; }
log_error()   { echo -e "${RED}[ERROR]${NC} $*"; }

header() {
    echo ""
    echo -e "${BOLD}${CYAN}$*${NC}"
    echo -e "${DIM}$(printf '%.0s─' $(seq 1 40))${NC}"
}

confirm() {
    local msg="${1:-Continue?}"
    local default="${2:-n}"
    local prompt
    if [[ "$default" == "y" ]]; then
        prompt="[Y/n]"
    else
        prompt="[y/N]"
    fi
    echo -ne "${YELLOW}$msg $prompt ${NC}"
    read -r answer
    answer="${answer:-$default}"
    [[ "$answer" =~ ^[Yy] ]]
}

load_env() {
    local env_file="${DOCKWEB_ROOT}/.env"
    if [[ -f "$env_file" ]]; then
        set -a
        set -f  # disable glob expansion
        source <(grep -v '^\s*#' "$env_file" | grep -v '^\s*$')
        set +f
        set +a
    fi
}

validate_domain() {
    local domain="$1"
    if [[ "$domain" =~ ^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?)*\.[a-zA-Z]{2,}$ ]]; then
        return 0
    fi
    return 1
}

sanitize_domain() {
    echo "$1" | sed 's/[.\-]/_/g'
}

get_local_domain() {
    # example.com -> example.local
    # sub.example.com -> sub.example.local
    echo "${1%.*}.local"
}

get_site_conf() {
    local domain="$1"
    local conf="${DOCKWEB_ROOT}/sites/${domain}/.dockweb.conf"
    if [[ -f "$conf" ]]; then
        source "$conf"
        return 0
    fi
    return 1
}

list_all_sites() {
    local sites=()
    local conf
    for conf in "${DOCKWEB_ROOT}"/sites/*/.dockweb.conf; do
        [[ -f "$conf" ]] || continue
        local domain
        domain=$(grep '^DOMAIN=' "$conf" | cut -d= -f2)
        [[ -n "$domain" ]] && sites+=("$domain")
    done
    if [[ ${#sites[@]} -gt 0 ]]; then
        printf '%s\n' "${sites[@]}"
    fi
}

site_count() {
    list_all_sites | wc -l
}

# Unique PHP container names currently referenced by any site
list_php_containers() {
    local conf c
    local -a containers=()
    for conf in "${DOCKWEB_ROOT}"/sites/*/.dockweb.conf; do
        [[ -f "$conf" ]] || continue
        c=$(grep '^PHP_CONTAINER=' "$conf" | cut -d= -f2)
        [[ -n "$c" ]] && containers+=("$c")
    done
    [[ ${#containers[@]} -eq 0 ]] && return 0
    printf '%s\n' "${containers[@]}" | sort -u
}

# Domains hosted by a given PHP container (one per line)
sites_in_container() {
    local container="$1"
    local conf c d
    for conf in "${DOCKWEB_ROOT}"/sites/*/.dockweb.conf; do
        [[ -f "$conf" ]] || continue
        c=$(grep '^PHP_CONTAINER=' "$conf" | cut -d= -f2)
        d=$(grep '^DOMAIN=' "$conf" | cut -d= -f2)
        [[ "$c" == "$container" ]] && echo "$d"
    done
    # Don't leak the last `[[ ]]` test's exit status — callers run under
    # `set -e`, and a final non-match (exit 1) would kill the script.
    return 0
}

docker_compose_cmd() {
    local cmd="docker compose -f ${DOCKWEB_ROOT}/docker-compose.yml -f ${DOCKWEB_ROOT}/docker-compose.sites.yml"
    # Brokers compose is auto-generated by `dockweb broker add`; absent if no
    # site has a broker yet. Append it so all dockweb subcommands see the
    # broker services without users having to fiddle with COMPOSE_FILE.
    if [[ -f "${DOCKWEB_ROOT}/docker-compose.brokers.yml" ]]; then
        cmd="${cmd} -f ${DOCKWEB_ROOT}/docker-compose.brokers.yml"
    fi
    echo "$cmd"
}

generate_password() {
    openssl rand -base64 24 | tr -d '/+=' | head -c 32
}

# Map user-friendly name to docker compose service name.
# Accepts: nginx, mysql, redis, a domain name, or a PHP container name.
# For a domain, returns its PHP_CONTAINER value (the compose service name).
resolve_service() {
    local input="$1"
    case "$input" in
        nginx|mysql|redis|backup|adminer|monitor|fail2ban|logrotate|certbot|modsecurity)
            echo "$input"
            return 0
            ;;
    esac
    # Try as domain name
    if [[ -f "${DOCKWEB_ROOT}/sites/${input}/.dockweb.conf" ]]; then
        grep '^PHP_CONTAINER=' "${DOCKWEB_ROOT}/sites/${input}/.dockweb.conf" | cut -d= -f2
        return 0
    fi
    # Try as a PHP container name that some site references
    local conf c
    for conf in "${DOCKWEB_ROOT}"/sites/*/.dockweb.conf; do
        [[ -f "$conf" ]] || continue
        c=$(grep '^PHP_CONTAINER=' "$conf" | cut -d= -f2)
        if [[ "$c" == "$input" ]]; then
            echo "$input"
            return 0
        fi
    done
    return 1
}

check_dependencies() {
    local missing=()
    command -v docker &>/dev/null || missing+=("docker")
    docker compose version &>/dev/null || missing+=("docker-compose-plugin")
    command -v openssl &>/dev/null || missing+=("openssl")
    if [[ ${#missing[@]} -gt 0 ]]; then
        log_error "Missing dependencies: ${missing[*]}"
        log_info "Run 'dockweb setup' to install them."
        return 1
    fi
    return 0
}

is_running() {
    # `docker compose ps -q --status running` prints one container ID per
    # running service and nothing else — robust across compose versions,
    # unlike grepping the human-readable STATUS column ("Up 5 minutes" vs
    # "running" depending on version). `|| true` stops a non-zero exit
    # from compose (e.g. no project yet) from tripping `set -e`.
    local cmd ids
    cmd="$(docker_compose_cmd)"
    ids=$($cmd ps -q --status running 2>/dev/null || true)
    [[ -n "$ids" ]]
}

ensure_sites_compose() {
    local f="${DOCKWEB_ROOT}/docker-compose.sites.yml"
    if [[ ! -f "$f" ]]; then
        cat > "$f" <<'EOF'
# Auto-generated by dockweb - DO NOT EDIT MANUALLY
services: {}

networks:
  web_network:
    driver: bridge
EOF
    fi
}
